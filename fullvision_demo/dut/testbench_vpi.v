// Testbench with VPI Trace Collection
// 使用VPI插件采集trace，同时生成golden VCD
...
module testbench;
    ...
    // Golden VCD - 采集所有DUT内部信号
    initial begin
`ifdef GOLDEN_VCD_FILE
        $dumpfile(`GOLDEN_VCD_FILE);
`else
        $dumpfile("golden.vcd");
`endif
        $dumpvars(0, dut);  // 只dump dut，包含所有内部信号
    end

    // VPI Trace采集 - 在仿真开始时初始化
    initial begin
